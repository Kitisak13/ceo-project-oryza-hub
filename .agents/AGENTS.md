...
- Power BI DAX Creation skill: `builtin/skills/pbi-dax-create/SKILL.md` — natural language DAX creation based on actual model
- Power BI Model Review skill: `builtin/skills/pbi-modelo-review/SKILL.md` — model audit, anti-patterns and performance review
- Power BI Documentation skill: `builtin/skills/pbi-doc/SKILL.md` — model and project documentation
...
### Mandatory Pre-Flight for Power BI & PBIP Projects
- ⚠️ **MANDATORY**: At the start of any Power BI / PBIP project session or before authoring PBIP files, DAX measures, or report visuals, the agent MUST read and align with the following skills:
  1. `powerbi-report-authoring` & `powerbi-report-design` (for PBIR/PBIP file mechanics, schema validity, and visual layout)
  2. `pbi-dax-create` (for DAX measure generation matching actual model schemas)
  3. `pbi-modelo-review` (for model audit, performance and anti-pattern validation)
